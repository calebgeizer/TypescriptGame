export default class Primitive {
    ...
    createElement(element:string,id:string,left?,top?) {
        var svgns = this.getSvgNS();
        const result = document.createElementNS(svgns, element);
        result.setAttribute('id', id);
        if (typeof left !== "undefined") result.setAttribute("x", left);
        if (typeof top !== "undefined") result.setAttribute("y", top);
    ...
    use(id,href,left?,top?,color?){
        ...
        if (typeof color !== "undefined") use.setAttribute("fill", color);
